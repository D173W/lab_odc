...
pelota:
    ...
    //  (x, y) = (x1, x2) posicion pelota
    //  (x3, x4) vector direccion
    //  
    //  
    //
    //
    ...
    //                   ______________
    //                 |               |
    //  tope izquierdo |  . <- pelota  | tope derecho
    //                 |               |
    //               /  --------------- <---- tope inferior
    //              /    mesa ping pong    \
    //             /------------------------\
    ...
    mov x1, 169 // posicion x de pelota
	mov x2, 258 // posicion y de pelota
    mov x3, 3  // inicializacion del vector direccion        
    mov x4, 8                                              
    ...
    bl read_display // leemos la pantalla para despues tapar la pelota previa en cada fotograma
    

loop_pelota:

    cmp x1, 446
    b.hi cambio_direccion_i // si me pase del tope derecho
    bl skip_cambio_direccion_i
        cambio_direccion_i:
            mov x3, -10 // le pega nacho
            mov x4, 4
            
    skip_cambio_direccion_i:

    cmp x1, 190 
    b.lo cambio_direccion_d // si me pase del tope izquierdo
    bl skip_cambio_direccion_d
    cambio_direccion_d:
        
        mov x3, 8 // le pega valen
        mov x4, 3
        
    skip_cambio_direccion_d:

    cmp x2, 271 
    b.hi pica // si me pase del tope inferior
    bl skip_pica
        pica:
            sub x4, xzr, x4 // cambiar a direccion y opuesta, porque pica en la mesa.
            

    skip_pica:

    cmp x2, 240
    b.lo cambio_direccion_y // si me pase del tope superior

    bl pintar_pelota

    cambio_direccion_y:
       
        sub x4, x4, 1 // gradualmente bajar la pelota, como si fuese gravedad.
        bl pintar_pelota
...
pintar_pelota:
    ...
    movz x7, 0x0040, lsl 16
    movk x7, 0x0000, lsl 00 // "framebuffer"
    mov x5, x1
    mov x6, x2 // (x5, x6) guardan los valores de la pelota en el fotograma pasado
